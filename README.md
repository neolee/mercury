...
**Strong recommendation**: Tencent's Hunyuan translation-specialized model [MT-1.5 1.8B](https://huggingface.co/tencent/HY-MT1.5-1.8B-GGUF) is a great fit here. Running it locally as your primary translation model is highly recommended. NOTE: you should set **Prompt Strategy** to **HY-MT Optimized** in **Translation** agent setting if you use this model series.
...
**特别推荐**：混元的 [MT-1.5 1.8B](https://huggingface.co/tencent/HY-MT1.5-1.8B-GGUF) 翻译专用模型非常适合这里，强烈建议尝试在本地运行该模型作为翻译的主力模型。注意，使用这个系列的翻译模型时，请在翻译智能体设置中将 **提示词策略** 设为 **HY-MT 优化**。
