...
enum DigestExportPolicy {
    ...
    static func renderSingleEntryMarkdownFallback(
        _ content: DigestSingleEntryMarkdownContent,
        bundle: Bundle
    ) -> String {
        let sourceLabel = String(localized: "Source", bundle: bundle)
        let authorLabel = String(localized: "Author", bundle: bundle)
        let noteLabel = String(localized: "My Take", bundle: bundle)
        let generatedPrefix = String(localized: "Generated by", bundle: bundle)
        let generatedSuffix = String(localized: "in", bundle: bundle)
        let slug = makeSingleEntryFileSlug(title: content.digestTitle)

        var lines: [String] = [
            "+++",
            "date = '\(iso8601Formatter.string(from: content.exportDate))'",
            "draft = false",
            "title = '\(content.digestTitle)'",
            "slug = '\(slug)'",
            "+++",
            "",
            "**\(sourceLabel)**: [\(content.articleTitle)](\(content.articleURL))",
            "**\(authorLabel)**: \(content.articleAuthor)"
        ]

        if let summaryText = content.summaryText,
           let targetLanguage = content.summaryTargetLanguage,
           let detailLevel = content.summaryDetailLevel {
            lines.append("")
            lines.append("> \(blockquoteBody(summaryText))")
            lines.append(">")
            lines.append(
                "> - \(generatedPrefix) [Mercury Summary Agent](https://github.com/neolee/mercury) \(generatedSuffix) (`\(targetLanguage)`, `\(detailLevel.rawValue)`)"
            )
        }

        if let noteText = content.noteText {
            lines.append("")
            lines.append("**\(noteLabel)**: \(noteText)")
        }
