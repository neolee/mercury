...
private enum ProviderUsageReportState {
    case loading
...
struct ProviderUsageReportView: View {
    ...
    @State private var reportState: ProviderUsageReportState = .loading
    @State private var hoveredBucket: ProviderUsageDailyBucket?

    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                Text("Provider Statistics", bundle: bundle)
                    .font(.title2)
                    .fontWeight(.semibold)

                Text(context.providerName)
                    .font(.title3)
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        Chart {
                            ForEach(snapshot.dailyBuckets) { bucket in
                                ...
                                .foregroundStyle(by: .value("Token Type", "Up"))
                                ...
                                .position(by: .value("Series", "Tokens"))
                                .foregroundStyle(by: .value("Token Type", "Down"))
                        ...
                        .chartForegroundStyleScale([
                            "Up": Color.accentColor,
                            "Down": Color.secondary.opacity(0.6)
                        ])
                        .chartYAxis {
...
private enum UsageReportTaskAggregation: Hashable {
    case all
...
private extension UsageReportWindowPreset {
    var labelKey: LocalizedStringKey {
        switch self {
        case .last1Week:
            "Last 1 Week"
